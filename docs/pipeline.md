# Конвейер и CI/CD

## Получение исходных данных

Отдельный `scripts/benchmark.py` выполняет реальный CPU-бенчмарк и записывает сырые измерения в `data/measurements.csv`. Код измерения хранится в том же репозитории, поэтому набор данных можно воспроизвести командой:

```bash
python scripts/benchmark.py
```

Бенчмарк намеренно **не запускается при каждой сборке сайта**: время выполнения зависит от текущей нагрузки машины. В Git фиксируется конкретный измеренный набор данных вместе с метаданными окружения, а CI воспроизводимо строит публикацию именно для этой версии данных.

## Локальная сборка

После создания виртуального окружения достаточно выполнить:

```bash
pip install -r requirements.txt
make build
```

Команда `make build` сначала запускает `scripts/generate.py`, затем выполняет `mkdocs build --strict`.

Для локального просмотра:

```bash
make serve
```

## GitHub Actions

Workflow запускается при push в `master`, при pull request и вручную.

```text
checkout
  ↓
Python + зависимости
  ↓
восстановление кэша эксперимента
  ↓
generate.py
  ↓
mkdocs build --strict
  ↓
Pages artifact
  ↓
deploy (только master)
  ↓
healthcheck опубликованного сайта
```

Для pull request выполняется только проверка и сборка. Публикация выполняется только из основной ветки `master`.

### Два способа публикации GitHub Pages

| Способ | Суть |
|---|---|
| Push в `gh-pages` | workflow складывает готовый HTML в отдельную ветку, из которой Pages публикует сайт |
| `upload-pages-artifact` + `deploy-pages` | готовый каталог сайта передаётся как Pages-артефакт и публикуется официальным механизмом GitHub |

В работе выбран второй вариант: он не создаёт служебную ветку с результатом сборки и напрямую поддерживается GitHub Pages.

## Кэширование P3

Ключ результата вычисляется из:

- `data/measurements.csv`;
- `data/benchmark_metadata.json`;
- `scripts/benchmark.py`;
- `scripts/generate.py`;
- `requirements.txt`.

Если эти файлы не изменились, агрегированные значения и PNG берутся из `.cache/experiment`. В GitHub Actions каталог `.cache/experiment` сохраняется между запусками с помощью `actions/cache`.

Это позволяет не пересчитывать неизменившийся эксперимент, но метаданные страницы (commit и дата сборки) всё равно обновляются.

## Проверка после публикации

После успешного deployment workflow автоматически проверяет опубликованный сайт:

1. главная страница отвечает без HTTP-ошибки и содержит контрольную строку `Публикация результатов исследования`;
2. страница `/experiment/` открывается и содержит заголовок P3 и таблицу результатов;
3. файл `/assets/experiment.png` доступен и имеет ненулевой размер.

Для запросов используется `curl --fail` с повторными попытками, чтобы учесть небольшую задержку публикации GitHub Pages. Если любая из проверок не проходит, workflow завершается с ошибкой.
